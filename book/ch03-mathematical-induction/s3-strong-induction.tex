...
\begin{cproof}
The fact that $p(n)$ is true for all $n \ge n_1$ follows from strong induction. Indeed:
\begin{itemize}
\item $p(n_1)$ is true by (i);
\item Fix $n \ge n_1$ and assume $p(k)$ is true for all $n_1 \le k \le n$. Then in fact $p(k)$ is true for all $n_0 \le k \le n$, since $p(n_0), p(n_0+1), \dots p(n_1-1)$ are true by (i). So $p(n+1)$ is true by (ii)
\end{itemize}
So $p(n)$ is true for all $n \ge n_1$. But then $p(n)$ is true for all $n \ge n_0$, again by (i).
\end{cproof}
