...
The \textbf{existential quantifier} is the quantifier $\exists$ \inlatex{exists}\inlatex{exists}{$\exists$}; if $p(x)$ is a logical formula with free variable $x$ with range $X$, then $\exists x \in X,\, p(x)$ is the logical formula defined according to the following rules:
\begin{itemize}
