...
\tldritem{thmLogicalEquivalentIffSameTruthValues}
Two \textit{propositional} formulae are logically equivalent if and only if they have the same truth value for all assignments of truth values to their constituent propositional variables.

\tldritem{defTruthTable}
The \textit{truth table} of a propositional formula is a tabulation of its truth values under all assignments of truth values to its constituent propositional variables. If the formula contains $n$ variables, then the truth table will have $2^n$ rows in addition to the header row. Two propositional formulae are logically equivalent if and only if they have identical columns in a truth table.
\end{tldrlist}
